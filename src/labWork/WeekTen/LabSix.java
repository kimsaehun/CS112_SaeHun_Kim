...
public class LabSix {// and Homework 11
	public static void main(String[] args) {
		...
		while (playing) {
			...
			values.add(dice.Throw(numOfDices, 1));// throw the num of dices but
													// only 1 times
			...
			// calculate who wins
			if ((pGuesses.get(round - 1) == values.get(round - 1))
					&& (cGuesses.get(round - 1) == values.get(round - 1))) {
				System.out
				...
				if (cGuesses.get(round - 1) == values.get(round - 1)) {
